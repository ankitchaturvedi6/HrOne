...
function fetchData(pageNo) {
    ...
    xmlhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (pageNo !== 1) {
                // console.log(document.getElementById('content-table'));
                document.getElementById("con-table").innerHTML = this.responseText;
                // showAvailablePages();
            } else {
                ...
                // console.log(`reposnse ${jsonContent.maxPage}`);
                showNoOfPages();
                // console.log(document.getElementById("con-table"));

                document.getElementById("con-table").innerHTML = jsonContent.values;

            }
        }
    };
    const url = document.getElementById('page-url').value;
    // console.log(`${url}?Page="${pageNo}"&Row="${noOfRows}" `)
    xmlhttp.open("GET", `${url}?Page=${pageNo}&Row=${noOfRows} `, true);
    xmlhttp.send();
}
